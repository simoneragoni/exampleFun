...
# Building `AliCascadeAnalysis_v0`
...
$ make
... verbose output ...
[100%] Built target runCascadeAnalysis
...
$
```

Again, the exact output will be system specific, but you should see the
`runCascadeAnalysis` target built without error. The resulting `mpags-cipher`
executable can then be run directly, and provides the following command
line options:

```
$ ./runCascadeAnalysis --help
Usage: runCascadeAnalysis [-i <file>] [-o <file>] [-k <key>] [--encrypt/--decrypt]
...
If no input file is supplied, `runCascadeAnalysis` will wait for user input
...
# Copying
`runCascadeAnalysis` is licensed under the terms of the MIT License. Please see
